export const projectsData = [
  {
    id: "01",
    title: "pit0nisa",
    title_en: "pit0nisa",
    title_es: "pit0nisa",
    categoryLabel: "LITERATURA EXPANDIDA",
    categoryLabel_en: "EXPANDED LITERATURE",
    categoryLabel_es: "LITERATURA EXPANDIDA",
    category: "literatura",
    description: `pit0nisa es una antología que explora la naturaleza mediúmnica del lenguaje computacional. Generados a partir de un algoritmo de cadenas de Markov, los poemas de pit0nisa son excusas para hacer hablar a la máquina sobre sí misma. La dimensión cinemática de la carga marca su propio ritmo: el mensaje no puede ser acelerado, tampoco retardado; estamos ante un oráculo aleatorio y arbitrario.
      \n
      Este proyecto fue desarrollado en el taller *Escribir como máquinas*, de Matías Buonfrate y Gerardo Montoya, e incluido en la antología de [Lit(e)Lat 2024](https://litelat.net/). 
      `,
    description_es: `pit0nisa es una antología que explora la naturaleza mediúmnica del lenguaje computacional. Generados a partir de un algoritmo de cadenas de Markov, los poemas de pit0nisa son excusas para hacer hablar a la máquina sobre sí misma. La dimensión cinemática de la carga marca su propio ritmo: el mensaje no puede ser acelerado, tampoco retardado; estamos ante un oráculo aleatorio y arbitrario.
      \n
      Este proyecto fue desarrollado en el taller *Escribir como máquinas*, de Matías Buonfrate y Gerardo Montoya, e incluido en la antología de [Lit(e)Lat 2024](https://litelat.net/). 
      `,
    description_en: `pit0nisa is an anthology that explores the mediumistic nature of computational language. Generated with a Markov-chain algorithm, its poems are an excuse to make the machine speak about itself. The cinematic dimension of the payload sets its own rhythm: the message can be neither accelerated nor slowed down; we face a random, arbitrary oracle.
      \n
      This project was developed in the workshop *Writing Like Machines* by Matías Buonfrate and Gerardo Montoya, and included in the [Lit(e)Lat 2024](https://litelat.net/) anthology.
      `,
    image: "/images/pit0nisa.png",
    link: "https://pit0nisa.netlify.app/",
    year: "2020",
    technologies: ["HTML", "CSS", "JavaScript"],
  },
  {
    id: "02",
    title: "mc-txt",
    title_en: "mc-txt",
    title_es: "mc-txt",
    categoryLabel: "LITERATURA EXPANDIDA",
    categoryLabel_en: "EXPANDED LITERATURE",
    categoryLabel_es: "LITERATURA EXPANDIDA",
    category: "literatura",
    description: `
      mc-txt es un generador de texto gamificado. A partir de búsquedas en la API de Wikipedia, mc-txt recibe extractos relacionados a los inputs ingresados y utiliza cadenas de Markov para generar texto nuevo.
      \n
      Este proyecto fue desarrollado en el marco del taller *Escribir como máquinas*, de Matías Buonfrate y Gerardo Montoya, e incluido en la antología de [Lit(e)Lat 2024](https://litelat.net/). 
      `,
    description_es: `
      mc-txt es un generador de texto gamificado. A partir de búsquedas en la API de Wikipedia, mc-txt recibe extractos relacionados a los inputs ingresados y utiliza cadenas de Markov para generar texto nuevo.
      \n
      Este proyecto fue desarrollado en el marco del taller *Escribir como máquinas*, de Matías Buonfrate y Gerardo Montoya, e incluido en la antología de [Lit(e)Lat 2024](https://litelat.net/). 
      `,
    description_en: `
      mc-txt is a gamified text generator. Based on searches in the Wikipedia API, mc-txt receives extracts related to the provided inputs and uses Markov chains to generate new text.
      \n
      The project was developed in the workshop *Writing Like Machines* by Matías Buonfrate and Gerardo Montoya, and included in the [Lit(e)Lat 2024](https://litelat.net/) anthology.
      `,
    image: "/images/mc-txt.png",
    link: "https://mc-txt.netlify.app/",
    year: "2021",
    technologies: ["React", "Nes.CSS"],
  },
  {
    id: "03",
    title: "botkowski",
    title_en: "botkowski",
    title_es: "botkowski",
    categoryLabel: "BOT",
    categoryLabel_en: "BOT",
    categoryLabel_es: "BOT",
    category: "bots",
    description: `
    @_botkowski es un bot de X inspirado en ["My Computer"](https://allpoetry.com/My-Computer), de Charles Bukowski. En este poema, el autor cuenta las reacciones de su entorno luego de la compra de su primera computadora: sus colegas y hasta su editor temían que la máquina terminara escribiendo su obra. 
    \n
    En "My computer", Bukowski responde: 
    \`me? I want to go the next step beyond the computer\`
    \n
    
  Este bot cumple esa ambición: extrae vocabulario de *Essential Bukowski: Poetry* y recombina sus palabras para crear contenido original en X, manteniendo su gramática y estilo.
  \n
  El bot nació en el taller *Jardín de Bots*, coordinado por [Canek Zapata](https://canekzapata.net/index.html). 
      `,
    description_es: `
    @_botkowski es un bot de X inspirado en ["My Computer"](https://allpoetry.com/My-Computer), de Charles Bukowski. En este poema, el autor cuenta las reacciones de su entorno luego de la compra de su primera computadora: sus colegas y hasta su editor temían que la máquina terminara escribiendo su obra. 
    \n
    En "My computer", Bukowski responde: 
    \`me? I want to go the next step beyond the computer\`
    \n
    
  Este bot cumple esa ambición: extrae vocabulario de *Essential Bukowski: Poetry* y recombina sus palabras para crear contenido original en X, manteniendo su gramática y estilo.
  \n
  El bot nació en el taller *Jardín de Bots*, coordinado por [Canek Zapata](https://canekzapata.net/index.html). 
      `,
    description_en: `
    @_botkowski is an X bot inspired by ["My Computer"](https://allpoetry.com/My-Computer) by Charles Bukowski. In that poem, he describes the reactions after buying his first computer: colleagues—and even his editor—feared the machine would end up writing his work. 
    \n
    In "My computer", Bukowski replies: 
    \`me? I want to go the next step beyond the computer\`
    \n
    
  This bot fulfills that ambition: it extracts vocabulary from *Essential Bukowski: Poetry* and recombines the words to create original content on X, preserving his grammar and style.
  \n
  The bot was born in the *Bot Garden* workshop, coordinated by [Canek Zapata](https://canekzapata.net/index.html).
      `,
    image: "/images/botkowski.png",
    link: "https://x.com/_botkowski",
    year: "2022",
    technologies: ["Gimmick bots", "No-code"],
  },
  {
    id: "04",
    title: "budín-cam",
    title_en: "budín-cam",
    title_es: "budín-cam",
    categoryLabel: "IOT",
    categoryLabel_en: "IOT",
    categoryLabel_es: "IOT",
    category: "iot",
    description: `
    BudínCam es un sistema de monitoreo hogareño construido con una ESP32-CAM conectada a WiFi y enlazada a un bot de Telegram. 
    
    A través de comandos como \`/photo\` o \`/flash\`, permite la captura remota de imágenes y control del flash integrado.
    
    El proyecto fue pensado para chequear a mi gata cuando no estoy en casa, especialmente, que su alimentador automático siga funcionando.
    `,
    description_es: `
    BudínCam es un sistema de monitoreo hogareño construido con una ESP32-CAM conectada a WiFi y enlazada a un bot de Telegram. 
    
    A través de comandos como \`/photo\` o \`/flash\`, permite la captura remota de imágenes y control del flash integrado.
    
    El proyecto fue pensado para chequear a mi gata cuando no estoy en casa, especialmente, que su alimentador automático siga funcionando.
    `,
    description_en: `
    BudínCam is a home monitoring system built with an ESP32-CAM connected to WiFi and linked to a Telegram bot.
    
    Through commands like \`/photo\` or \`/flash\`, it allows remote image capture and control of the built-in flash.
    
    The project was designed to check on my cat when I'm away—especially to ensure her automatic feeder keeps working.
    `,
    image: "/images/esp32.jpeg",
    link: "",
    year: "2024",
    technologies: ["ESP32-CAM", "Telegram Bot", "IoT"],
    images: [
      {
        src: "/budin-cam/budin-cam-1.jpeg",
        alt_es: "Budín con la ESP32-CAM",
        alt_en: "Budín with the ESP32-CAM",
        caption_es: "Budín con la ESP32-CAM",
        caption_en: "Budín with the ESP32-CAM",
      },
      {
        src: "/budin-cam/budin-cam-2.jpeg",
        alt_es: "Prueba de flash de la ESP32-CAM",
        alt_en: "ESP32-CAM flash test",
        caption_es: "Prueba de flash de la ESP32-CAM",
        caption_en: "ESP32-CAM flash test",
      },
      {
        src: "/budin-cam/budin-cam-3.jpeg",
        alt_es: "Montaje en pared",
        alt_en: "Wall mounting",
        caption_es: "Montaje en pared",
        caption_en: "Wall mounting",
      },
      {
        src: "/budin-cam/budin-cam-4.jpeg",
        alt_es: "Capturas del bot de Telegram",
        alt_en: "Telegram bot screenshots",
        caption_es: "Capturas del bot de Telegram",
        caption_en: "Telegram bot screenshots",
      },
    ]
  },
  {
    id: "05",
    title: "mdq-cyborg",
    title_en: "mdq-cyborg",
    title_es: "mdq-cyborg",
    categoryLabel: "ELECTRÓNICA",
    categoryLabel_en: "ELECTRONICS",
    categoryLabel_es: "ELECTRÓNICA",
    category: "hardware",
    description: `
    mdq-cyborg es un souvenir marplatense reimaginado: una estación climática casera que consulta una API para obtener datos de la temperatura y humedad como los clásicos lobos marinos del clima. 
    
    El dispositivo consta de una placa ESP-WROOM, conectada a tres LEDs que son los que traducen la data de la API e indican el nivel de humedad: azul (normal), violeta (media) y rosa (alta). Su v1 tenía, además, un sensor de temperatura y humedad DHT22, que luego fue reemplazado por la API Open-Meteo.
    `,
    description_es: `
    mdq-cyborg es un souvenir marplatense reimaginado: una estación climática casera que consulta una API para obtener datos de la temperatura y humedad como los clásicos lobos marinos del clima. 
    
    El dispositivo consta de una placa ESP-WROOM, conectada a tres LEDs que son los que traducen la data de la API e indican el nivel de humedad: azul (normal), violeta (media) y rosa (alta). Su v1 tenía, además, un sensor de temperatura y humedad DHT22, que luego fue reemplazado por la API Open-Meteo.
    `,
    description_en: `
    mdq-cyborg is a reimagined Mar del Plata souvenir: a DIY weather station that queries an API to obtain temperature and humidity data—akin to the classic sea lion weather figurines.
    
    The device is built around an ESP-WROOM board connected to three LEDs that translate the API data into humidity levels: blue (normal), violet (medium), and pink (high). Its v1 also featured a DHT22 temperature/humidity sensor, later replaced by the Open-Meteo API.
    `,
    image: "/images/mdq-cyborg-16.jpeg",
    link: "",
    year: "2024",
    technologies: ["ESP-WROOM", "Clima API", "LEDs", "Electrónica"],
    images: [
      {
        src: "/mdq-cyborg/mdq-cyborg-1.jpeg",
        alt_es: "Esquemático en Fritzing (v1)",
        alt_en: "Fritzing schematic (v1)",
        caption_es: "Esquemático en Fritzing (v1)",
        caption_en: "Fritzing schematic (v1)",
      },
      {
        src: "/mdq-cyborg/mdq-cyborg-4.jpeg",
        alt_es: "Transferencia térmica del diseño",
        alt_en: "Thermal transfer of the design",
        caption_es: "Transferencia térmica del diseño",
        caption_en: "Thermal transfer of the design",
      },
      {
        src: "/mdq-cyborg/mdq-cyborg-8.jpeg",
        alt_es: "Grabado en ácido férrico",
        alt_en: "Etching with ferric chloride",
        caption_es: "Grabado en ácido férrico",
        caption_en: "Etching with ferric chloride",
      },
      {
        src: "/mdq-cyborg/mdq-cyborg-5.jpeg",
        alt_es: "Limpieza de la placa en agua",
        alt_en: "Cleaning the board in water",
        caption_es: "Limpieza de la placa en agua",
        caption_en: "Cleaning the board in water",
      },
      {
        src: "/mdq-cyborg/mdq-cyborg-7.jpeg",
        alt_es: "Resultado del grabado",
        alt_en: "Etching result",
        caption_es: "Resultado del grabado",
        caption_en: "Etching result",
      },
      {
        src: "/mdq-cyborg/mdq-cyborg-11.jpeg",
        alt_es: "Placa transferida",
        alt_en: "Transferred board",
        caption_es: "Placa transferida",
        caption_en: "Transferred board",
      },
      {
        src: "/mdq-cyborg/mdq-cyborg-12.jpeg",
        alt_es: "Posicionamiento de los LEDs para soldado",
        alt_en: "Positioning LEDs for soldering",
        caption_es: "Posicionamiento de los LEDs para soldado",
        caption_en: "Positioning LEDs for soldering",
      },
      {
        src: "/mdq-cyborg/mdq-cyborg-13.jpeg",
        alt_es: "Soldado de componentes",
        alt_en: "Soldering components",
        caption_es: "Soldado de componentes",
        caption_en: "Soldering components",
      },
      {
        src: "/mdq-cyborg/mdq-cyborg-17.jpeg",
        alt_es: "Resultado del soldado",
        alt_en: "Soldering result",
        caption_es: "Resultado del soldado",
        caption_en: "Soldering result",
      },
      {
        src: "/mdq-cyborg/mdq-cyborg-19.jpeg",
        alt_es: "Sensor de temperatura y humedad quemado (v1)",
        alt_en: "Burned temperature and humidity sensor (v1)",
        caption_es: "Sensor de temperatura y humedad quemado (v1)",
        caption_en: "Burned temperature and humidity sensor (v1)",
      },
      {
        src: "/mdq-cyborg/mdq-cyborg-20.jpeg",
        alt_es: "mdq-cyborg en un día de buen clima (1)",
        alt_en: "mdq-cyborg on a good weather day (1)",
        caption_es: "mdq-cyborg en un día de buen clima (1)",
        caption_en: "mdq-cyborg on a good weather day (1)",
      },
      {
        src: "/mdq-cyborg/mdq-cyborg-21.jpeg",
        alt_es: "mdq-cyborg en un día de buen clima (2)",
        alt_en: "mdq-cyborg on a good weather day (2)",
        caption_es: "mdq-cyborg en un día de buen clima (2)",
        caption_en: "mdq-cyborg on a good weather day (2)",
      },
      {
        src: "/mdq-cyborg/mdq-cyborg-22.jpeg",
        alt_es: "mdq-cyborg en un día de mucha humedad",
        alt_en: "mdq-cyborg on a humid day",
        caption_es: "mdq-cyborg en un día de mucha humedad",
        caption_en: "mdq-cyborg on a humid day",
      },
    ],
  },
  {
    id: "06",
    title: "script-fusión",
    title_en: "script-fusión",
    title_es: "script-fusión",
    categoryLabel: "INSTALACIÓN",
    categoryLabel_en: "INSTALLATION",
    categoryLabel_es: "INSTALACIÓN",
    category: "creative-coding",
    description: `
    Proyecto final para la Maestría en Artes Electrónicas (UNTREF). La web del proyecto fue desarrollada en Next.js, frontend y backend, y conectada por Bluetooth a una impresora térmica. El backend actúa como proceso padre y controla un submódulo que replica, mediante ingeniería inversa, la app original de la impresora. Al repositorio original se le sumaron funciones como la impresión sin mantener ratio de la imagen (para imprimir, por ejemplo, tickets largos, como las recetas que utilizamos para el proyecto) y desactivar el modo suspensión (para matener funcionando la impresora durante la exposición).  
    \n
    El proyecto fue expuesto en [Casa Belgrado](https://www.instagram.com/casabelgrado/) durante la muestra [La pulsión de lo errático](https://www.instagram.com/p/DDBDIrpvue7/).
    `,
    description_es: `
    Proyecto final para la Maestría en Artes Electrónicas (UNTREF). La web del proyecto fue desarrollada en Next.js, frontend y backend, y conectada por Bluetooth a una impresora térmica. El backend actúa como proceso padre y controla un submódulo que replica, mediante ingeniería inversa, la app original de la impresora. Al repositorio original se le sumaron funciones como la impresión sin mantener ratio de la imagen (para imprimir, por ejemplo, tickets largos, como las recetas que utilizamos para el proyecto) y desactivar el modo suspensión (para matener funcionando la impresora durante la exposición).  
    \n
    El proyecto fue expuesto en [Casa Belgrado](https://www.instagram.com/casabelgrado/) durante la muestra [La pulsión de lo errático](https://www.instagram.com/p/DDBDIrpvue7/).
    `,
    description_en: `
    Final project for the Master's in Electronic Arts (UNTREF). The project's website was built in Next.js—frontend and backend—and connected via Bluetooth to a thermal printer. The backend acts as a parent process and controls a submodule that replicates, through reverse engineering, the printer's original app. We added features such as printing without preserving aspect ratio (to print, for example, long tickets like the recipes we used) and disabling sleep mode (to keep the printer running during the exhibition).
    \n
    The project was exhibited at [Casa Belgrado](https://www.instagram.com/casabelgrado/) during the show [La pulsión de lo errático](https://www.instagram.com/p/DDBDIrpvue7/).
    `,
    image: "/images/scriptfusion.png",
    link: "https://script-fusion.vercel.app/",
    year: "2024",
    technologies: [
      "Next.js",
      "Bluetooth",
      "Thermal Printer",
      "Node.js",
      "Reverse Engineering",
    ],
  },
  {
    id: "07",
    title: "bucles",
    title_en: "loops",
    title_es: "bucles",
    categoryLabel: "ELECTRÓNICA",
    categoryLabel_en: "ELECTRONICS",
    categoryLabel_es: "ELECTRÓNICA",
    category: "hardware",
    description: `

bucles está construido sobre una placa Arduino Mega 2560 a la que están conectados, a través de una protoboard, los siguientes componentes:

- Una pantalla lcd de 16x2, soldada al módulo I2C
- Un potenciómetro de 100k ohms 
- Una serie de seis leds, organizados en forma de círculo

El potenciómetro controla la frecuencia de los procesos. El lcd recibe la orden y despacha el poema al ritmo solicitado. Los leds reaccionan en el mismo sentido, cubriendo un loop completo. La comunicación entre los componentes sucede de forma cíclica.

El proyecto recupera el poema de [@lafatbordieu](https://www.instagram.com/lafatbordieu/) que encapsula el ritmo de las cosas y su recursividad. En el concepto de bucles extraños, según dice Douglas Hofstadter, autor de *Gödel Escher Bach: un eterno y grácil bucle*, va implícito el de infinito, porque qué otra cosa es un bucle sino una forma de representar de manera finita un proceso interminable. 


    `,
    description_es: `
 
 bucles está construido sobre una placa Arduino Mega 2560 a la que están conectados, a través de una protoboard, los siguientes componentes:
 
 - Una pantalla lcd de 16x2, soldada al módulo I2C
 - Un potenciómetro de 100k ohms 
 - Una serie de seis leds, organizados en forma de círculo
 
 El potenciómetro controla la frecuencia de los procesos. El lcd recibe la orden y despacha el poema al ritmo solicitado. Los leds reaccionan en el mismo sentido, cubriendo un loop completo. La comunicación entre los componentes sucede de forma cíclica.
 
 El proyecto recupera el poema de [@lafatbordieu](https://www.instagram.com/lafatbordieu/) que encapsula el ritmo de las cosas y su recursividad. En el concepto de bucles extraños, según dice Douglas Hofstadter, autor de *Gödel Escher Bach: un eterno y grácil bucle*, va implícito el de infinito, porque qué otra cosa es un bucle sino una forma de representar de manera finita un proceso interminable. 
 
 
    `,
    description_en: `
 
 loops is built on an Arduino Mega 2560 board with the following components connected via a breadboard.
 
 - A 16x2 LCD, soldered to the I2C module
 - A 100k-ohm potentiometer 
 - A series of six LEDs arranged in a circle
 
 The potentiometer controls the processes' frequency. The LCD receives the command and dispatches the poem at the requested tempo. The LEDs react accordingly, completing a full loop. Communication between components happens cyclically.
 
 The project references a poem by [@lafatbordieu](https://www.instagram.com/lafatbordieu/) that encapsulates the rhythm of things and their recursiveness. In the concept of strange loops, as Douglas Hofstadter (author of *Gödel, Escher, Bach: An Eternal Golden Braid*) says, infinity is implicit—since what else is a loop but a way to finitely represent an endless process?
 
 
    `,
    image: "/images/bucles1.png",
    link: "https://vimeo.com/1091489697?share=copy#t=0",
    video: "https://vimeo.com/1091489697?share=copy#t=0",
    year: "2024",
    technologies: ["Arduino Mega 2560", "LCD", "LEDs", "Poesía"],
  },
  {
    id: "08",
    title: "page turner DIY",
    title_en: "page turner DIY",
    title_es: "page turner DIY",
    categoryLabel: "ELECTRÓNICA",
    categoryLabel_en: "ELECTRONICS",
    categoryLabel_es: "ELECTRÓNICA",
    category: "hardware",
    description: `
En abril me pedí por Amazon un page turner con control remoto para el Kindle, junto con otros dispositivos para leer cómoda desde el sillón.MEl envío se retrasó… después se volvió a retrasar… y al final lo cancelaron y me ofrecieron el reembolso. Así que pensé en armar uno DIY.

Usé un Arduino Pro Mini, un sensor IR, un micro servo y un control remoto genérico. El primer obstáculo fue mínimo pero clásico: el control no traía pila. Se resolvió con una batería reciclada de otro dispositivo.

La pantalla del Kindle es capacitiva, así que el toque del servo necesitaba conductividad. Le envolví el brazo en papel aluminio y lo conecté a 3.3V para simular un dedo.

El resultado: un page turner DIY, funcional, preciso y cómodo para leer sin moverse del sillón.

    `,
    description_es: `
 En abril me pedí por Amazon un page turner con control remoto para el Kindle, junto con otros dispositivos para leer cómoda desde el sillón.MEl envío se retrasó… después se volvió a retrasar… y al final lo cancelaron y me ofrecieron el reembolso. Así que pensé en armar uno DIY.
 
 Usé un Arduino Pro Mini, un sensor IR, un micro servo y un control remoto genérico. El primer obstáculo fue mínimo pero clásico: el control no traía pila. Se resolvió con una batería reciclada de otro dispositivo.
 
 La pantalla del Kindle es capacitiva, así que el toque del servo necesitaba conductividad. Le envolví el brazo en papel aluminio y lo conecté a 3.3V para simular un dedo.
 
 El resultado: un page turner DIY, funcional, preciso y cómodo para leer sin moverse del sillón.
 
    `,
    description_en: `
 In April I ordered a remote-controlled Kindle page turner on Amazon, along with other devices to read comfortably from the couch. The shipment was delayed... then delayed again... and eventually canceled with a refund. So I decided to build a DIY one.
 
 I used an Arduino Pro Mini, an IR sensor, a micro servo, and a generic remote control. The first hurdle was small but classic: the remote had no battery. I solved it with a recycled battery from another device.
 
 The Kindle screen is capacitive, so the servo's tap needed conductivity. I wrapped the arm in aluminum foil and connected it to 3.3V to simulate a finger.
 
 The result: a DIY page turner—functional, precise, and comfortable for reading without moving from the couch.
 
    `,
    image: "/images/page-turner-diy-1.jpeg",
    video: "https://vimeo.com/1092307749?share=copy",
    year: "2025",
    technologies: ["Arduino Pro Mini", "IR sensor", "Micro Servo Motor"],
  },
];
